...
import type { Bin } from "@HA/services/staking/schema";
...
const categories = computed((): string[] =>
  bins.map((bin) => {
    const minValueFormatted = bin.minValueUsd
      ? `${round(bin.minValueUsd, 0, "dollar")}${unit(bin.minValueUsd)}`
      : "0";
    const maxValueFormatted = bin.maxValueUsd
      ? `${round(bin.maxValueUsd, 0, "dollar")}${unit(bin.maxValueUsd)}`
      : "0";
