...
export async function getAprHistory(
  ...
  const data = await fetch(
    `${host}/v1/savings/${chain}/apr/history${queryString}`,
    undefined,
    options?.signal
  );
...
export async function getTvlHistory(
  ...
  const queryString = addQueryString(valid);
  const data = await fetch(
    `${host}/v1/savings/${chain}/tvl/history${queryString}`,
    undefined,
    options?.signal
  );
...
const getVolumeHistoryParams = z.object({
  chain: z.string(),
  start: z.number().optional(),
  end: z.number().optional(),
  page: z.number().optional(),
...
export async function getVolumeHistory(
  params: z.infer<typeof getVolumeHistoryParams>,
  options?: Options
) {
  const host = getHost(options);
  ...
  const queryString = addQueryString(valid);
  const data = await fetch(
    `${host}/v1/savings/${chain}/volume/history${queryString}`,
    undefined,
    options?.signal
  );
...
export async function getEvents(
  ...
  const data = await fetch(
    `${host}/v1/savings/${chain}/events${queryString}`,
    undefined,
    options?.signal
  );
...
export async function getTopUsers(
  ...
  const data = await fetch(
    `${host}/v1/savings/${chain}/top_users`,
    undefined,
    options?.signal
  );
