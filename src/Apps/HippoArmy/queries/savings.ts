...
export function useStats(params: MaybeRefOrGetter<{ chain: string }>) {
  return useQuery({
    queryKey: ["savings-stats", params] as const,
    queryFn: () => Api.getStats(toValue(params)),
  });
}
...
export function useEvents(
  params: MaybeRefOrGetter<{ chain: string; page?: number; per_page?: number }>
) {
  return useQuery({
    queryKey: ["savings-events", params] as const,
...
export function useTopUsers(params: MaybeRefOrGetter<{ chain: string }>) {
  return useQuery({
    queryKey: ["savings-top-users", params] as const,
