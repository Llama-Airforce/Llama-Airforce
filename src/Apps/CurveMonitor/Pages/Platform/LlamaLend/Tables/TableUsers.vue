...
import type { MarketUser } from "@CM/queries/llamalend";

const { users, collateralSymbol } = defineProps<{
  users: MarketUser[];
...
const columns = [
  ...
  { id: "borrowed", label: "Borrowed", sort: true, align: "end" } as const,
...
.table {
  --columns-data: auto 8rem minmax(6rem, 1fr) minmax(6rem, 1fr)
    minmax(6rem, 1fr) minmax(6rem, 1fr) minmax(6rem, 1fr) minmax(3rem, 1fr) 8rem;
}
