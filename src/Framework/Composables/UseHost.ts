  ...
  if (isDevelopment) {
    try {
      ...
      if (response.ok) {
        ...
        notify({
          text: "Local development server available but didn't ping back correctly from HEAD, using production host.",
          type: "warn",
        });
      }
    } catch (error) {
      notify({
        text: "Local development server not available, using production host.",
        type: "warn",
      });
      cachedHost = hostProd;
    }
